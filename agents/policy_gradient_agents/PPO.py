...
class PPO(Base_Agent):
    ...
    def step(self):
        ...
        return float(self.losses),self.agent_name
    ...
    def policy_learn(self):
        ...
        for _ in range(self.hyperparameters["learning_iterations_per_round"]):
            ...
            self.loss = self.calculate_loss([all_ratio_of_policy_probabilities], all_discounted_returns)
            self.losses=self.loss.cpu().detach().numpy()
            self.take_policy_new_optimisation_step(self.loss)
