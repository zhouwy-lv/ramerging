    ...
    def run_games_for_agents(self):
        ...
        for agent_number, agent_class in enumerate(self.agents):
            ...
            if self.config.visualise_overall_agent_results:
                agent_rolling_score_results = [results[1] for results in  self.results[self.agent_name]]
                self.visualise_overall_agent_results(agent_rolling_score_results, self.agent_name, show_mean_and_std_range=True)
