...
class Base_Agent(object):
    ...
    def __init__(self, config):
        ...
        self.log_game_info()
        self.is_sucessbuffer=deque(maxlen=100)
        self.is_final_successbuffer=deque(maxlen=100)
        self.is_dangerbuffer = deque(maxlen=100)
        self.is_crashbuffer = deque(maxlen=100)
        self.num_dangeraction = deque(maxlen=100)
        self.time_outbuffer = deque(maxlen=100)
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        while self.episode_number < num_episodes:
            # self.loss=[]
            self.reset_game()
            self.agent_name=self.step()
            # self.step()
            self.is_sucessbuffer.append(self.info["is_success"])
            self.is_final_successbuffer.append(self.info['is_final_sccess'])
            self.is_crashbuffer.append(self.info['crash'])
            self.is_dangerbuffer.append(self.info['is_danger'])
            if self.config.env_title == "RampMerging-v1":
                self.num_dangeraction.append(self.info['numdanger_action'])
            self.time_outbuffer.append(self.info['time_out'])
            self.total_episode_score_so_far += self.reward
            if save_and_print_results: self.save_and_print_result()
            if self.config.save_model and self.episode_number % self.config.save_model_freq == 0:
    ...
    def print_rolling_result(self):
        ...
        # text = """"\r Episode {0}, Score: {3: .3f}, Max score seen: {4: .3f}, Rolling score: {1: .3f}, Max rolling score seen: {2: .3f}"""

        # if self.episode_number > 4:
        if self.agent_name=="DDPG" or "TD3":

            text = """"\r Episode {0}, Score: {2: .3f}, Rolling score: {1: .3f}"""
            ...
            sys.stdout.write(text.format(self.episode_number, self.rolling_results[-1],
                                         self.game_full_episode_scores[-1]))
            # self.writer.add_scalar('loss/DDPG_crit_loss',self.crit_loss,self.episode_number)
            # self.writer.add_scalar('loss/DDPG_act_loss', self.act_loss, self.episode_number)

        elif self.agent_name=="PPO":
            text = """"\r Episode {0}, Score: {2: .3f}, Rolling score: {1: .3f}, loss: {3: .3f}"""
            sys.stdout.write(text.format(len(self.game_full_episode_scores), self.rolling_results[-1],
                                         self.game_full_episode_scores[-1], self.loss))
            self.writer.add_scalar('loss/PPO_loss',self.loss,self.episode_number)

        elif self.agent_name=="SAC":
            # if self.hyperparameters["automatically_tune_entropy_hyperparameter"]:
            #     self.qf1los, self.qf2los, self.actor_loss, self.alphaloss =self.loss
            #
            #     # self.writer.add_scalar('loss/SAC_alphaloss',self.alphaloss,self.episode_number)
            # else:
            #     self.qf1los, self.qf2los, self.actor_loss = self.loss
            #     self.alphaloss = None
            # text = """"\r Episode {0}, Score: {2: .3f}, Rolling score: {1: .3f}, qf1loss: {3: .3f}, qf2loss: {4: .3f}, actloss: {5: .3f},alpha_loss{6: .3f}"""
            # sys.stdout.write(text.format(self.episode_number, self.rolling_results[-1],
            #                              self.game_full_episode_scores[-1], self.qf1los,self.qf2los,self.actor_loss,self.alphaloss))
            text = """"\r Episode {0}, Score: {2: .3f}, Rolling score: {1: .3f}"""
            sys.stdout.write(text.format(self.episode_number, self.rolling_results[-1],
                                         self.game_full_episode_scores[-1]))
            # self.writer.add_scalar('loss/SAC_qf1loss',self.qf1los,self.episode_number)
            # self.writer.add_scalar('loss/SAC_qf2loss',self.qf2los,self.episode_number)
            # self.writer.add_scalar('loss/SAC_actorloss',self.actor_loss,self.episode_number)
        # print('/n step:{}'.format(self.global_step_number))
        if self.episode_number % 20 == 0:
            self.writer.add_scalar('rewards/rolling_results',self.rolling_results[-1],self.episode_number)
            self.writer.add_scalar('rewards/game_full_episode_scores', self.game_full_episode_scores[-1], self.episode_number)
            self.writer.add_scalar('performance/num_successmean', np.mean(self.is_sucessbuffer), self.episode_number)
            self.writer.add_scalar('performance/num_finalsuccessmean', np.mean(self.is_final_successbuffer), self.episode_number)
            self.writer.add_scalar('performance/num_crashmean', np.mean(self.is_crashbuffer),self.episode_number)
            self.writer.add_scalar('performance/num_isdangermean', np.mean(self.is_dangerbuffer),self.episode_number)
            if self.config.env_title == "RampMerging-v1":
                self.writer.add_scalar('performance/num_dangeraction', np.mean(self.num_dangeraction), self.episode_number)
            self.writer.add_scalar('performance/timeout', np.mean(self.time_outbuffer), self.episode_number)
            sys.stdout.write("  successmean:{} final_successmea:{}".format(np.mean(self.is_sucessbuffer),np.mean(self.is_final_successbuffer)))
